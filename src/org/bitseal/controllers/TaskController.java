...
public class TaskController
{
	 ...
	 */
	public int checkForMessagesAndSendAcks()
	{
		...
		{
			Log.e(TAG, "While running TaskController.checkForMessagesAndSendAcks(), SendMessageController.checkServerForMessages() threw a RuntimeExecption. \n" +
					"The exception message was: " + runEx.getMessage());
		}
	
		int newMessagesReceived = processIncomingMessages();
		
		// If any new messages were successfully processed, run the 'send acknowledgments' task
		if (newMessagesReceived > 0)
		{
			// Create a new QueueRecord for the 'send acknowledgments' task
			QueueRecordProcessor queueProc = new QueueRecordProcessor();
			QueueRecord queueRecord1 = queueProc.createAndSaveQueueRecord(BackgroundService.TASK_SEND_ACKS, null, null);
			
			// First check whether an Internet connection is available. If not, the QueueRecord for the
			// 'send acknowledgments' task will be saved (as above) and processed later
			if (NetworkHelper.checkInternetAvailability() == true)
			{
				// Send any pending acknowledgments
				sendAcknowledgments(queueRecord1);
			}
		}
	...
	/**
	 * Checks the database for any Payload objects containing new msgs. 
	 * 
	 * @return An int representing the number of new messages successfully 
	 * processed (zero to many)
	 */
	...
	/**
	 * Attempts to send any outstanding acknowlegments for messages
	 * that I have received. <br><br>
	 * 
	 * @param queueRecord0 - A QueueRecord object for the task of sending
	 * all outstanding acknowledgments
	 */
	public void sendAcknowledgments(QueueRecord queueRecord0)
	{
		...
		boolean allAcksProcessedSuccessfully = false;
		try
		{
			CheckForMessagesController controller = new CheckForMessagesController();
			allAcksProcessedSuccessfully = controller.sendAcknowledgments();
		}
		catch (RuntimeException runEx)
		{
			Log.e(TAG, "While running TaskController.sendAcknowledgments(), CheckForMessagesController.sendAcknowledgments() threw a RuntimeExecption. \n" +
					"The exception message was: " + runEx.getMessage());
			queueProc.updateQueueRecordAfterFailure(queueRecord0); // Update the QueueRecord to record the failed attempt
			return; // If we failed to send all our outstanding acknowledgments because of an exception, leave the QueueRecord for that
					  // task in place so that it can be attempted again later
		}
		
		// If we successfully sent our acknowledgments, delete the QueueRecord for that task. Otherwise,
		// update the QueueRecord to record the failed attempt.
		if (allAcksProcessedSuccessfully == true)
		{
			queueProc.deleteQueueRecord(queueRecord0);
		}
		else
